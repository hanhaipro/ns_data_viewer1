...
import { ColumnWidthInput } from './components/column-width-input/ColumnWidthInput.jsx';
import { FilterForm } from './components/filter-form/FilterForm.jsx';
import { Loading } from './components/loading/Loading.jsx';
import { orderRequest } from './requests/order.request.js';
...
const App = () => {
  ...
  const [rangeFillState, setRangeFillState] = useState(() => ({
    isOpen: false,
    startRowId: null,
    field: null,
    targetRowNumber: '',
  ...
  const loading = useMemo(() => loadingData || loadingTotal, [loadingData, loadingTotal]);

  useEffect(() => {
    const getTotal = async () => {
      try {
        setLoadingTotal(true);
        const { total } = await orderRequest.total(filterConditions);
        setTotalItems(total);
      } catch {
      } finally {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const query = {
          ...
          keyword: searchText,
          ...filterConditions,
          ...sortConfig.map((e) => `${e.key}::${e.direction}`),
        };
        const { items } = await orderRequest.list(query);
        setData(items);
      } catch {
      } finally {
    ...
    getData();
  }, [filterConditions, sortConfig, searchText, currentPage, itemsPerPage]);
  ...
  const checkIfColumnFiltered = (colId) => {
    switch (colId) {
      case 'pol':
        return filterConditions.pol !== 'all';
      case 'pod':
        return filterConditions.pod !== 'all';
      case 'country':
        return filterConditions.country !== 'all';
      case 'customer':
        return filterConditions.customer !== 'all';
      case 'etd':
        return !!(filterConditions.etdFrom || filterConditions.etdTo);
      case 'blDate':
        return !!(filterConditions.blDateFrom || filterConditions.blDateTo);
      case 'flagDereg':
        return filterConditions.flagDereg !== 'all';
      case 'flagOdo':
        return filterConditions.flagOdo !== 'all';
      default:
        return false;
    }
  };
  ...
  const getRowWarnings = (row) => {
    ...
    if (warningSettings.missingFlagEnabled && (row.flagDereg === '未' || row.flagOdo === '未'))
      warnings.push('Flag未完了');
  ...
  const fillToBottom = (rowId, field) => {
    ...
    const newData = data.map((r, index) => {
      if (index > currentIndex) {
        targetRowIds.push(r.id);
        return { ...r, [field]: valueToCopy };
      }
      return r;
    });
    setData(newData);
  ...
  const executeRangeFill = () => {
    ...
    if (isNaN(targetIndex) || targetIndex <= startIndex || targetIndex >= data.length) {
      setRangeFillState({ ...rangeFillState, isOpen: false });
      return;
    }
      ...
      <>
        {parts.map((part, i) =>
          part.toLowerCase() === highlight.toLowerCase() ? (
            <span key={i} className="bg-amber-300 text-black font-bold rounded-[2px] px-0.5 mx-[-2px]">
              {part}
            </span>
          ) : (
            part
  ...
  const updateColumnWidthState = (colId, width) => {
    setColumns((prev) => prev.map((col) => (col.id === colId ? { ...col, width: parseInt(width) || 0 } : col)));
  };
  const toggleColumnVisibility = (colId) => {
    setColumns((prev) => prev.map((col) => (col.id === colId ? { ...col, visible: !col.visible } : col)));
  };
  const toggleColumnSticky = (colId) => {
    setColumns((prev) => prev.map((col) => (col.id === colId ? { ...col, sticky: !col.sticky } : col)));
  };
  ...
  return (
    <div
      className="flex flex-col h-screen w-full bg-slate-100 font-sans text-[12px] text-slate-800 overflow-hidden"
      style={{ color: displaySettings.textColor }}
    >
      {/* 画面ヘッダー */}
      <header
        className="flex items-center justify-between px-4 h-14 text-white shadow-lg z-30 shrink-0"
        style={{ backgroundColor: displaySettings.headerColor }}
      >
        ...
        <div className="flex items-center gap-1.5">
                  ...
                  {!isCreatingView ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={saveNewView}
                          className="flex-1 py-1 bg-indigo-600 text-white text-xs rounded font-bold hover:bg-indigo-700"
                        >
                          保存
                        </button>
                        <button
                          onClick={() => setIsCreatingView(false)}
                          className="flex-1 py-1 bg-slate-200 text-slate-600 text-xs rounded font-bold hover:bg-slate-300"
                        >
                          キャンセル
                        </button>
                      </div>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-md transition-all ${activeTab === 'filter' && settingsOpen ? 'bg-white/20 ring-1 ring-white/40' : 'hover:bg-white/10'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-md transition-all ${activeTab === 'warning' && settingsOpen ? 'bg-white/20 ring-1 ring-white/40' : 'hover:bg-white/10'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-md transition-all ${activeTab === 'columns' && settingsOpen ? 'bg-white/20 ring-1 ring-white/40' : 'hover:bg-white/10'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-md transition-all ${activeTab === 'highlight' && settingsOpen ? 'bg-white/20 ring-1 ring-white/40' : 'hover:bg-white/10'}`}
          >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {/* メインテーブル */}
        <div className="flex-1 overflow-auto bg-white relative" id="table-scroll-parent">
          ...
          >
            <thead className="sticky top-0 z-20 shadow-md">
              <tr>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between truncate gap-1">
                          ...
                          {canSort && (
                            <div
                              className={`shrink-0 flex items-center ${isSorted ? 'opacity-100 text-amber-300' : 'opacity-30'}`}
                            >
                              {/* 複数ソート時の番号表示 */}
                        ...
                        <div
                          ...
                          className="absolute right-0 top-0 h-full w-2 cursor-col-resize hover:bg-white/30 z-30"
                        />
                      </th>
                    );
                  })}
                <th
                  style={{ backgroundColor: displaySettings.headerColor }}
                  className="w-12 sticky right-0 z-20 border-b-2 border-white/20"
                ></th>
              </tr>
            </thead>
            <tbody>
              {data.map((row, rowIndex) => {
                ...
                const rowHeightClass =
                  displaySettings.rowDensity === 'standard'
                    ? 'h-10'
                    : displaySettings.rowDensity === 'compact'
                      ? 'h-7'
                      : 'h-6';
                ...
                return (
                  <tr
                    ...
                    style={{
                      backgroundColor:
                        isHighlight && !isActive && !isActionHighlight ? displaySettings.highlightColor : undefined,
                    }}
                  >
                    {columns
                      .filter((c) => c.visible)
                      .map((col, index, array) => {
                        ...
                        const isTargeting =
                          rangeFillState.isOpen &&
                          rangeFillState.startRowId === row.id &&
                          rangeFillState.field === col.id;
                        let leftPos = 0;
                        if (col.sticky) {
                          for (let i = 0; i < index; i++) {
                            if (array[i].sticky) leftPos += array[i].width;
                        ...
                        const filterHighlightClass = isColFiltered
                          ? 'bg-yellow-100 font-bold text-slate-800 px-1 rounded'
                          : '';
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`flex items-center ${col.id === 'rowNo' ? 'justify-center' : 'justify-between'} group/cell relative overflow-hidden h-full`}
                            >
                              <div className="flex items-center gap-1.5 truncate w-full">
                                ...
                                {isEditing && !col.sticky && col.id !== 'rowNo' ? (
                                  ...
                                  <span className={`truncate w-full ${filterHighlightClass}`}>
                                    {col.id === 'country' ? (
                                      <span>
                                        {getFlagEmoji(value)} {highlightText(value, searchText)}
                                      </span>
                                    ) : col.id === 'flagDereg' || col.id === 'flagOdo' ? (
                                      <span
                                        className={`flex items-center gap-1 font-medium ${value === '完了' ? 'text-emerald-600' : 'text-slate-400'}`}
                                      >
                              ...
                              {isEditing && !col.sticky && col.id !== 'rowNo' && (
                                <div className="hidden group-hover/cell:flex items-center gap-0.5 absolute right-0 top-[-2px] z-50 bg-white shadow-sm border border-slate-200 rounded p-0.5 scale-90">
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      copyFromAbove(row.id, col.id);
                                    }}
                                    className="p-1 bg-indigo-50 text-indigo-600 rounded hover:bg-indigo-600 hover:text-white"
                                    title="上からコピー"
                                  >
                                    <Copy size={10} />
                                  </button>
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      fillToBottom(row.id, col.id);
                                    }}
                                    className="p-1 bg-emerald-50 text-emerald-600 rounded hover:bg-emerald-600 hover:text-white"
                                    title="下すべて"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="p-1 bg-sky-50 text-sky-600 rounded hover:bg-sky-600 hover:text-white"
                                    title="指定行まで"
                                  >
                                    <ListPlus size={10} />
                                  </button>
                              ...
                              {isTargeting && (
                                <div className="absolute inset-0 bg-white z-[60] flex items-center px-1 shadow-inner border border-sky-500 rounded">
                                  ...
                                  <input
                                    type="number"
                                    value={rangeFillState.targetRowNumber}
                                    onChange={(e) =>
                                      setRangeFillState({ ...rangeFillState, targetRowNumber: e.target.value })
                                    }
                                    className="w-10 text-[10px] border border-slate-200 rounded px-1 outline-none focus:border-sky-500"
                                    autoFocus
                                    onKeyDown={(e) => {
                                      if (e.key === 'Enter') executeRangeFill();
                                      if (e.key === 'Escape') setRangeFillState({ ...rangeFillState, isOpen: false });
                                    }}
                                  />
                                  <button
                                    onClick={executeRangeFill}
                                    className="ml-1 p-0.5 bg-sky-500 text-white rounded"
                                  >
                                    <Check size={10} />
                                  </button>
                                </div>
                    ...
                    <td className="w-12 px-1 flex items-center justify-center sticky right-0 bg-inherit shadow-[-1px_0_0_0_#e2e8f0] border-b border-slate-100 h-full">
                      <button
                        ...
                        className={`p-1 rounded-md transition-all ${isEditing ? 'bg-emerald-500 text-white shadow' : 'hover:bg-slate-200 text-slate-400 opacity-0 group-hover:opacity-100 hover:text-indigo-600'}`}
                      >
          ...
          <div className="w-80 border-l border-slate-200 bg-white shadow-2xl z-40 flex flex-col animate-in slide-in-from-right duration-200 overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/80 shrink-0">
              <h2 className="font-bold flex items-center gap-2 text-slate-700">
                {activeTab === 'filter' && (
                  <>
                    <Filter size={18} className="text-sky-500" /> 高度なフィルター
                  </>
                )}
                {activeTab === 'warning' && (
                  <>
                    <AlertTriangle size={18} className="text-rose-500" /> アラート設定
                  </>
                )}
                {activeTab === 'columns' && (
                  <>
                    <Columns size={18} className="text-indigo-500" /> 列表示・並び順
                  </>
                )}
                {activeTab === 'highlight' && (
                  <>
                    <Settings size={18} /> デザイン・UI設定
                  </>
                )}
              </h2>
              <button
            ...
            <div className="flex-1 p-5 space-y-6 overflow-y-auto">
              ...
              {activeTab === 'filter' && (
                <div className="space-y-6">
                  ...
                  <button
                    onClick={() => setFilterConditions(defaultFilterConditions)}
                    className="w-full py-2 text-[11px] text-rose-500 font-bold hover:bg-rose-50 rounded border border-rose-100 transition-all"
                  >
                    全フィルターリセット
                  </button>
                </div>
              ...
              {activeTab === 'warning' && (
                <div className="space-y-6">
                  {/* アラート項目 */}
                  <div
                    className={`p-4 ${currentAlertStyle.isCustom ? 'bg-slate-50' : currentAlertStyle.bg} border ${currentAlertStyle.isCustom ? 'border-slate-200' : currentAlertStyle.border} rounded-xl space-y-4 shadow-sm`}
                  >
                    <label className="flex items-center justify-between cursor-pointer">
                      <span
                        className={`text-xs font-bold ${currentAlertStyle.isCustom ? 'text-slate-800' : currentAlertStyle.title}`}
                      >
                        ETD警告 (日付接近)
                      </span>
                      <input
                        type="checkbox"
                        checked={warningSettings.etdEnabled}
                        onChange={() =>
                          setWarningSettings({ ...warningSettings, etdEnabled: !warningSettings.etdEnabled })
                        }
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        min="0"
                        max="30"
                        value={warningSettings.etdDays}
                        onChange={(e) => setWarningSettings({ ...warningSettings, etdDays: parseInt(e.target.value) })}
                        className="flex-1 accent-slate-400 h-1"
                      />
                      <span
                        className={`text-xs font-black w-12 text-right ${currentAlertStyle.isCustom ? 'text-slate-600' : currentAlertStyle.range}`}
                      >
                        {warningSettings.etdDays} 日内
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <label className="flex items-center justify-between p-3 border border-slate-100 rounded-lg hover:bg-slate-50 cursor-pointer transition-all">
                      ...
                      <input
                        type="checkbox"
                        checked={warningSettings.missingInvoiceEnabled}
                        onChange={() =>
                          setWarningSettings({
                            ...warningSettings,
                            missingInvoiceEnabled: !warningSettings.missingInvoiceEnabled,
                          })
                        }
                    ...
                    <label className="flex items-center justify-between p-3 border border-slate-100 rounded-lg hover:bg-slate-50 cursor-pointer transition-all">
                      ...
                      <input
                        type="checkbox"
                        checked={warningSettings.missingFlagEnabled}
                        onChange={() =>
                          setWarningSettings({
                            ...warningSettings,
                            missingFlagEnabled: !warningSettings.missingFlagEnabled,
                          })
                        }
                        className="accent-slate-600"
                      />
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      {/* カスタムカラー選択ボタン */}
                      <label
                        className={`flex-1 py-1.5 text-[10px] font-bold rounded-md border transition-all cursor-pointer flex items-center justify-center gap-1 ${warningSettings.alertTheme === 'custom' ? 'bg-slate-100 border-slate-300 text-slate-800 ring-1 ring-slate-300' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`}
                      >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col gap-0.5 mr-1 hidden sm:flex">
                            <button
                              ...
                              disabled={index === 0}
                              className="p-0.5 hover:bg-slate-100 rounded text-slate-300 hover:text-indigo-600 disabled:opacity-10"
                            >
                              <ArrowUp size={8} />
                            </button>
                            <button
                          ...
                          <button
                            ...
                            className={`p-1 rounded ${col.visible ? 'text-indigo-600 hover:bg-indigo-50' : 'text-slate-300'}`}
                          >
                            {col.visible ? <Eye size={14} /> : <EyeOff size={14} />}
                          </button>
                          <span
                            className={`text-[11px] ${col.visible ? 'text-slate-700 font-medium' : 'text-slate-400 line-through'}`}
                          >
                            {col.label}
                          </span>
                        </div>
                        {/* 4. 列幅入力の修正: 専用コンポーネントを使用 */}
                        <ColumnWidthInput colId={col.id} width={col.width} onUpdate={updateColumnWidthState} />
                      </div>
                    ))}
                  </div>
                  <p className="text-[10px] text-slate-400 px-1">
                    リストをドラッグ＆ドロップして列の順番を入れ替えられます。
                  </p>
                </div>
              )}
              ...
              {activeTab === 'highlight' && (
                <div className="space-y-6">
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {[
                        ...
                        <button
                          key={c.name}
                          onClick={() => setDisplaySettings({ ...displaySettings, headerColor: c.color })}
                          className={`w-8 h-8 rounded-lg border-2 transition-all ${displaySettings.headerColor === c.color ? 'border-sky-500 scale-105 shadow-md' : 'border-transparent opacity-70 hover:opacity-100'}`}
                          style={{ backgroundColor: c.color }}
                          title={c.name}
                        />
                      ))}
                      <label
                        className={`w-8 h-8 rounded-lg border-2 flex items-center justify-center cursor-pointer transition-all ${!['#1e293b', '#312e81', '#334155', '#064e3b'].includes(displaySettings.headerColor) ? 'border-sky-500 scale-105 shadow-md bg-white' : 'border-slate-200 bg-slate-100 hover:bg-slate-200'}`}
                        title="カスタムカラー"
                      >
                        ...
                        <input
                          ...
                          onChange={(e) => setDisplaySettings({ ...displaySettings, headerColor: e.target.value })}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          type="color"
                          value={displaySettings.borderColor}
                          onChange={(e) => setDisplaySettings({ ...displaySettings, borderColor: e.target.value })}
                          className="w-10 h-8 rounded border border-slate-200 cursor-pointer"
                        />
                        <span className="text-xs font-mono text-slate-500 bg-slate-100 px-2 py-1 rounded w-full text-center">
                          {displaySettings.borderColor}
                        </span>
                      </div>
                    </div>
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => setDisplaySettings({ ...displaySettings, textColor: e.target.value })}
                          className="w-10 h-8 rounded border border-slate-200 cursor-pointer"
                        />
                        <span className="text-xs font-mono text-slate-500 bg-slate-100 px-2 py-1 rounded w-full text-center">
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1">
                      <Palette size={12} /> ハイライト行の背景色
                    </label>
                    <div className="flex items-center gap-2">
                      <input
                        type="color"
                        value={displaySettings.highlightColor}
                        onChange={(e) => setDisplaySettings({ ...displaySettings, highlightColor: e.target.value })}
                        className="w-10 h-8 rounded border border-slate-200 cursor-pointer"
                      />
                      <span className="text-xs font-mono text-slate-500 bg-slate-100 px-2 py-1 rounded w-full text-center">
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1">
                      <Layers size={12} /> 行間の密度
                    </label>
                    <div className="flex bg-slate-100 p-1 rounded-lg">
                      {['standard', 'compact', 'x-compact'].map((d) => (
                        <button
                          key={d}
                          onClick={() => setDisplaySettings({ ...displaySettings, rowDensity: d })}
                          className={`flex-1 py-1.5 text-[10px] font-bold rounded-md transition-all ${displaySettings.rowDensity === d ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                      ...
                      <div
                        onClick={() =>
                          setDisplaySettings((prev) => ({
                            ...prev,
                            autoHighlightEnabled: !prev.autoHighlightEnabled,
                            highlightConditions: { ...prev.highlightConditions, active: !prev.autoHighlightEnabled },
                          }))
                        }
                        className={`w-10 h-5 rounded-full relative transition-all ${displaySettings.autoHighlightEnabled ? 'bg-indigo-600 shadow-indigo-200 shadow-md' : 'bg-slate-300'}`}
                      >
                        <div
                          className={`absolute top-1 w-3 h-3 bg-white rounded-full shadow transition-all ${displaySettings.autoHighlightEnabled ? 'left-6' : 'left-1'}`}
                        />
                      </div>
                    </label>
                      ...
                      <div className="bg-white border border-indigo-100 rounded-lg p-3 space-y-3 animate-in fade-in">
                        ...
                        <FilterForm
                          ...
                          onReset={() =>
                            setDisplaySettings((prev) => ({
                              ...prev,
                              highlightConditions: { ...defaultHighlightConditions, active: true },
                            }))
                          }
                          uniquePols={uniquePols}
                          uniquePods={uniquePods}
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 shrink-0">
              <button
                onClick={() => setSettingsOpen(false)}
                className="w-full py-3 bg-[#0f172a] text-white rounded-xl shadow-lg font-bold hover:bg-slate-800 transition-all"
              >
                設定を閉じて適用
              </button>
            </div>
      ...
      <footer className="h-10 bg-[#0f172a] border-t border-white/10 flex items-center justify-between px-4 text-[10px] text-white/50 z-30 shrink-0">
        <div className="flex gap-5 items-center">
          ...
          <span>
            全 {totalItems} 件中{' '}
            {totalItems > 0 ? `${startIndex + 1} - ${Math.min(startIndex + itemsPerPage, totalItems)}` : '0'} 件を表示
          </span>
        </div>
